...
rectum_count <- read.csv(file = "/storage/home/swashburn30/Helmsley/Batch1_13/MAGMA_analysis/data/helm_batch1_13_rectum_avg_gene_exp.csv")

rectum_count <- rectum_count %>% add_count(X) %>% 
  filter(n==1) %>%
  select(-n) %>%
  gather(key = column,value=Expr,-X) %>%
  as.tibble()

exp_agg <- rectum_count %>% rename(ClusterID=column, Expr_sum_mean=Expr)
...
write_tsv(exp_conti_spe, file = "rectum_conti_sepcificity_matrix.txt")
