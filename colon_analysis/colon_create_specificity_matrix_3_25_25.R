...
colon_count <- read.csv(file = "/storage/home/swashburn30/Helmsley/Batch1_13/MAGMA_analysis/data/helm_batch1_13_colon_avg_gene_exp.csv")

colon_count <- colon_count %>% add_count(X) %>% 
  filter(n==1) %>%
  select(-n) %>%
  gather(key = column,value=Expr,-X) %>%
  as.tibble()

exp_agg <- colon_count %>% rename(ClusterID=column, Expr_sum_mean=Expr)
...
write_tsv(exp_conti_spe, file = "colon_conti_sepcificity_matrix.txt")
